    ...
    for i, (curTime, curAnomaly) in enumerate(zip(timestamps, anomalyScores)):
      ...
      # If outside a window, score as if false positive
        ...
        if prevWindowRightIndex is None:
          ...
          # Fix for single-point anomaly windows (prevWindowWidth = 1)
          # When denominator is 0, treat the position as very far from the window
          if denominator == 0:
            # Single-point window: treat position as proportional to distance
            # Use the numerator directly as a measure of distance
            positionPastWindow = min(numerator, 10)  # Cap at 10 to avoid extreme values
          else:
